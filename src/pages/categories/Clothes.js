...
function Clothes({isUserPresentt}) {
    ...
    const getAllProducts = async () => {
      if(arrival === "oldest"){
        const res = await axios.get(
         `http://0.0.0.0:8000/api/products/?cat=C&new=False`
         );
         setProducts(res.data);
      }else{
        const res = await axios.get(
        "http://0.0.0.0:8000/api/products/?cat=C"
      );
      setProducts(res.data);
      }
    };
...
useEffect(() => {
  ...
  return (
    <main className="bg-gray-800 ">
      ...
      <Navbar isUserPresentt = {isUserPresentt} />
      <Slider />
      ...
      <div className="h-auto bg-gray-800 px-4 py-2 flex flex-col space-y-3 mt-20 mb-10 max-w-6xl mx-auto">
        ...
        <div className="flex items-center space-x-4 text-sm">
          ...
          >
            ...
            <option value="kid">Babies</option>
          </select>
          <select
          value={arrival}
      ...
      {!dCat && (
       ...
       <div className="grid grid-cols-4 max-w-6xl mx-auto gap-6 mb-10">
         {products.map((product)=>(
           ...
           rating={product.rating} img={product.image}/>
         ))}
       </div>
     </Fade>
     )}
